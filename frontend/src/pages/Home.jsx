...
const Home = () => {
  ...
  return (
    <div style={{ backgroundColor: 'var(--bg-base)', minHeight: '100vh', overflowX: 'hidden' }}>
      <nav className="px-6 py-5 flex items-center justify-between max-w-6xl mx-auto sticky top-0 z-40 backdrop-blur-sm" style={{ backgroundColor: 'color-mix(in srgb, var(--bg-base) 85%, transparent)' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/login" className="hidden sm:block text-sm font-medium px-4 py-2" style={{ color: 'var(--text-secondary)' }}>
            Log in
          </Link>
          <Link
            to="/register"
      ...
      <div className="max-w-4xl mx-auto px-6 pt-14 pb-10 text-center relative">
        ...
        <div
          ...
          style={{ backgroundColor: 'var(--highlight-soft)', animationDelay: '1s' }}
        />
        ...
        <div className="flex items-center justify-center gap-1.5 mb-12 text-xs" style={{ color: 'var(--text-muted)' }}>
          {[1, 2, 3, 4, 5].map((i) => (
            <Star key={i} size={13} fill="var(--highlight)" style={{ color: 'var(--highlight)' }} />
          ))}
      ...
      <Reveal className="max-w-4xl mx-auto px-6 pb-16">
        <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
          ...
          <div className="rounded-xl p-4 flex items-start gap-3" style={{ backgroundColor: 'var(--accent-soft)' }}>
            <Bot size={16} style={{ color: 'var(--accent)' }} className="mt-0.5 shrink-0" />
            <div>
      ...
      <div id="modules" className="max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { icon: MessageSquare, title: 'Ticket Vault & Live Chat', desc: 'Every customer ticket in one place with real-time two-way messaging between customer and assigned agent.' },
            { icon: Zap, title: 'AI Triage Engine', desc: 'Automatic category, priority, and summary suggestions generated the moment a ticket is submitted.' },
            { icon: BarChart3, title: 'Agent Analytics', desc: 'Live breakdown of ticket status and category distribution across the whole desk.' },
            { icon: ShieldCheck, title: 'Role-Based Access', desc: 'Customers see only their own tickets. Agents only manage what\'s assigned to them.' },
            { icon: Workflow, title: 'Guided Workflow', desc: 'New → Assigned → In Progress → Resolved. A resolved ticket stays locked until reopened.' },
            { icon: Bell, title: 'Live Status Updates', desc: 'Status changes and new replies appear automatically, no manual refresh required.' },
          ].map((m, idx) => {
            const Icon = m.icon;
            return (
      ...
      <div id="how-it-works" className="max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {[
            ...
            { icon: Bot, step: '02', title: 'AI triages instantly', desc: 'Category, priority, and a summary generated in seconds — ready for the agent to confirm or edit.' },
            { icon: CheckCircle2, step: '03', title: 'Agent resolves', desc: 'Assign, reply, and close — with the full conversation on record and updates in real time.' },
          ].map((f) => {
            const Icon = f.icon;
            return (
              <Reveal key={f.title}>
                <div
                  ...
                  style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}
                >
                  <span className="absolute top-4 right-5 text-4xl font-extrabold opacity-10" style={{ color: 'var(--accent)' }}>
                    {f.step}
                  </span>
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: 'var(--accent-soft)' }}>
                    <Icon size={20} style={{ color: 'var(--accent)' }} />
                  </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <Reveal>
            <div className="rounded-2xl p-7 h-full" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: 'var(--accent-soft)' }}>
                <Users size={22} style={{ color: 'var(--accent)' }} />
              </div>
              <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text-primary)' }}>For Customers</h3>
              <ul className="space-y-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                {['Submit tickets in under a minute', 'Track status of every ticket you\'ve raised', 'Message your assigned agent directly', 'Edit ticket details before it\'s resolved'].map((t) => (
          ...
          <Reveal>
            <div className="rounded-2xl p-7 h-full" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: 'var(--highlight-soft)' }}>
                <Lock size={22} style={{ color: 'var(--highlight)' }} />
              </div>
              <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text-primary)' }}>For Agents</h3>
              <ul className="space-y-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                {['Review and edit AI-suggested category & priority', 'Assign, reply, and resolve tickets from one dashboard', 'Real-time analytics on ticket volume and status', 'Reopen resolved tickets when needed'].map((t) => (
                  <li key={t} className="flex items-start gap-2">
                    <CheckCircle2 size={16} style={{ color: 'var(--highlight)' }} className="shrink-0 mt-0.5" />
                    {t}
                  </li>
                ))}
              </ul>
      ...
      <footer style={{ borderTop: '1px solid var(--border-color)' }}>
        <div className="max-w-5xl mx-auto px-6 py-12">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>SupportFlow</span>
              </div>
              <p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                AI-assisted customer support desk for managing tickets from submission to resolution.
              </p>
            </div>

            <div>
              <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-primary)' }}>Product</p>
            ...
            <div>
              <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-primary)' }}>Stack</p>
              <ul className="space-y-2 text-xs" style={{ color: 'var(--text-muted)' }}>
            ...
            <div>
              <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-primary)' }}>Project</p>
              <ul className="space-y-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          ...
          >
            ...
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              Crafted with care by <span className="font-medium" style={{ color: 'var(--accent)' }}>Sidra Aslam</span>
            </p>
          </div>
        </div>
      </footer>
